...
export default function Home() {
  ...
  const countryOptions = useMemo(() => liveCountries.length ? liveCountries.map((option) => ({ apiId: option.id, code: option.code.toLowerCase(), name: option.name, currency: option.currency?.symbol || option.currency?.code || "" })) : demoCountryOptions, [liveCountries]);
  const activeCountry = countryOptions.find((option) => option.code === country) || getDemoCountry(country);
  const heroLot = lots[0] || demoAuctionLots[0];
  ...
  return <main dir="rtl" className="min-h-screen overflow-x-hidden bg-[#f7f6f1] text-[#143039]">
    <div className="border-b border-white/10 bg-[#12313a] text-[#dce9e3]"><div className="market-container flex h-10 items-center justify-between text-xs"><span className="flex items-center gap-2"><ShieldCheck size={14} className="text-[#c7dcae]" /> {isLive ? "مزادات حية مقيدة بسياق الدولة" : "بيانات عرض مؤقتة حتى اتصال Laravel"}</span><label className="flex items-center gap-2"><Landmark size={14} /><span className="sr-only">اختر الدولة</span><select value={country} onChange={(event) => setCountry(event.target.value)} className="max-w-44 bg-transparent text-xs font-semibold outline-none">{countryOptions.map((option) => <option key={option.code} value={option.code}>{option.name}</option>)}</select></label></div></div>
    <header className="market-container flex h-20 items-center justify-between gap-5"><Link href="/" className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#d96d46] text-xl font-black text-white shadow-lg">م</span><span className="font-serif text-2xl font-semibold">مِزَاد</span></Link><nav className="hidden gap-7 text-sm font-semibold text-[#5b6d72] lg:flex"><a href="#auctions" className="text-[#12313a]">المزادات</a><a href="#discover">الاكتشاف</a><a href="#how">كيف يعمل</a><Link href="/sell">اعرض مقتناك</Link></nav><div className="flex items-center gap-2"><button onClick={() => toast.message("الإشعارات", { description: "سيتم ربط صندوق الإشعارات الحي في خطوة التكامل التالية." })} className="hidden h-10 w-10 place-items-center rounded-full border border-[#143039]/10 sm:grid" aria-label="إشعارات"><Bell size={18} /></button>{isAuthenticated ? <button onClick={() => navigate("/account")} className="rounded-full bg-[#12313a] px-5 py-2.5 text-sm font-bold text-white">{user?.name || "حسابي"}</button> : <button onClick={startLogin} disabled={loading} className="rounded-full bg-[#12313a] px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">دخول الحساب</button>}<button onClick={() => toast.message("القائمة", { description: "التنقل الكامل متاح من روابط الصفحة في هذه النسخة الأولية." })} className="grid h-10 w-10 place-items-center rounded-full border border-[#143039]/10 lg:hidden" aria-label="القائمة"><Menu size={18} /></button></div></header>
    <section className="market-container pb-16 pt-5 md:pb-24 md:pt-10">{liveError && <div className="mb-4 rounded-xl border border-[#d96d46]/30 bg-[#fff0e8] px-4 py-3 text-sm text-[#9d4027]">تعذر جلب المزادات الحية؛ نعرض بيانات العرض المؤقتة إلى أن تستعيد خدمة Laravel اتصالها.</div>}<div className="grid overflow-hidden rounded-[2rem] bg-[#12313a] shadow-[0_30px_90px_rgba(18,49,58,.2)] lg:grid-cols-[1.05fr_.95fr]"><div className="relative min-h-[31rem] p-8 text-[#fcfbf6] md:p-12"><div className="absolute inset-0 opacity-60 [background-image:radial-gradient(circle_at_12%_16%,rgba(199,220,174,.25),transparent_18%),radial-gradient(circle_at_78%_78%,rgba(217,109,70,.32),transparent_28%)]" /><div className="relative flex h-full flex-col items-start justify-between"><div><span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-bold text-[#c7dcae]"><Sparkles size={14} /> {activeCountry?.name ?? "سياق سوق مؤقت"}</span><h1 className="mt-7 font-serif text-5xl leading-[1.04] md:text-7xl">كل قطعة<br />لها قصتها.</h1><p className="mt-6 max-w-md text-base leading-8 text-[#cfe0dc]">منصة مزادات منتقاة تجمع القطع الاستثنائية مع مزايدة واضحة، ومدفوعات منظمة، وتسليم قابل للتتبع.</p></div><div className="mt-12 flex flex-wrap gap-3"><a href="#auctions" className="rounded-full bg-[#d96d46] px-6 py-3 text-sm font-bold text-white">استكشف المزادات</a><a href="#how" className="rounded-full border border-white/20 px-6 py-3 text-sm font-bold">كيف يعمل مزاد؟</a></div></div></div><div className="relative grid min-h-[26rem] place-items-center bg-[#d9cab7] p-5 md:p-8">{isLive && !liveError && liveAuctionsLoading ? <div className="rounded-2xl bg-[#fcfbf6]/95 p-8 text-center shadow-xl"><p className="font-serif text-2xl font-semibold">جارٍ تحميل المزادات الحية</p><p className="mt-2 text-sm text-[#61757a]">قد يستغرق إيقاظ الخادم التجريبي بضع ثوانٍ.</p></div> : isLive && !liveError && !lots.length ? <div className="rounded-2xl bg-[#fcfbf6]/95 p-8 text-center shadow-xl"><p className="font-serif text-2xl font-semibold">لا توجد مزادات حية حالياً</p><p className="mt-2 text-sm text-[#61757a]">أضف منتجاً معتمداً وجدول مزاداً ليظهر هنا.</p></div> : <><LotImage lot={heroLot} hero /><div className="absolute bottom-10 left-10 right-10 rounded-2xl bg-[#fcfbf6]/95 p-5 shadow-xl backdrop-blur"><p className="text-xs font-bold text-[#d96d46]">{heroLot.category}</p><div className="mt-2 flex items-end justify-between gap-3"><div><h2 className="font-serif text-2xl font-semibold">{heroLot.title}</h2><p className="mt-1 text-xs text-[#61757a]">{heroLot.city} · مزاد مباشر</p></div><strong className="shrink-0">{heroLot.price}</strong></div></div></>}</div></div></section>
